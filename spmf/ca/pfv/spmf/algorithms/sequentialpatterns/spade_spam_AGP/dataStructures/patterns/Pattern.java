 ...
 *
 * Copyright Antonio Gomariz Pe帽alver 2013
 *
...
public class Pattern implements Comparable<Pattern> {
    ...
    private double kValue = 0.0;

    private boolean signOfMatchList = true;
    private ArrayList<HashSet<String>> processCateList;
    /**
    ...
    public int getItemsSize() {
    	...
    	for (int i = 0; i < this.elements.size(); i++) {
			if (!elements.get(i).getAbstraction().hasEqualRelation()) {
				count++;
			}
		}
    	return count;
//    	return this.getElements().size();
	...
	public boolean isSignOfMatchList() {
		return signOfMatchList;
	}

	public void setSignOfMatchList(HashMap<Integer, String> i2categHashMap,int k) {
		processCateList = new ArrayList<HashSet<String>>();
		for (int i = 0; i < 6; i++) {
			processCateList.add(new HashSet<String>());
		}
		processCateList.get(0).add("检查费");processCateList.get(0).add("护理费");
		processCateList.get(1).add("诊查费");
		processCateList.get(2).add("检查费");processCateList.get(2).add("护理费");
		processCateList.get(3).add("检查费");processCateList.get(3).add("护理费");
		processCateList.get(4).add("检查费");processCateList.get(4).add("护理费");
		processCateList.get(5).add("诊查费");
//		int k = this.processCateList.size();

		this.signOfMatchList = true;
		if (this.getItemsSize() > processCateList.size()) {
			this.signOfMatchList = false;
		}
		else {
			for (int i = 0; i < Math.min(k, this.getItemsSize()); i++) {
				String itemString = elements.get(i).toString().replaceAll("[^0-9]", "");
				int it = Integer.valueOf(itemString);
				if(!processCateList.get(i).contains(i2categHashMap.get(it))) {
					this.signOfMatchList = false;
					break;
				}
			}
    ...
    public void setKValue(){
    	this.kValue = (double)appearingIn.cardinality()/(double)elements.size();
    }
	...
	public void setSignOfMatchList() {
		...
		else this.signOfMatchList = false;
	}
