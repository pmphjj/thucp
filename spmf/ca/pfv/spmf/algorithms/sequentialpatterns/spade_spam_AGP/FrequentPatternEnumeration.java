...
public class FrequentPatternEnumeration {
    ...
    private double maxKValue = 0.0;
    private ArrayList<Pattern> mileStonesList;
    /**
    ...
    public FrequentPatternEnumeration(CandidateGenerator candidateGenerator, double minSupportAbsolute, Saver saver) {
        ...
        this.saver = saver;
        this.mileStonesList = new ArrayList<Pattern>();
    }
    public void setCateMap(HashMap<Integer, String> i2categ) {
    	this.i2categHashMap = i2categ;
    }

    public ArrayList<Pattern> getMileStone() {
		return this.mileStonesList;
	}

    public double getMaxKValue() {
    	return this.maxKValue;
    }
    /**
     ...
     * @param coocMapBefore
     */
    public void execute(EquivalenceClass eq, boolean dfs, boolean keepPatterns, boolean verbose, Map<Integer, Map<Integer, Integer>> coocMapAfter, Map<Integer, Map<Integer, Integer>> coocMapEquals,int stepnumber) {
        /*eq.setIdList(null);
        ...
        for (int i = eqMembers.size() - 1; i >= 0; i--) {
            ...
            for (int j = i; j >= 0; j--) {
                ...
                for (Pattern extension : extensions) {
                    ...
                    if(INTERSECTION_COUNTER % 100000 == 0) System.out.println(INTERSECTION_COUNTER);
                    ...
                    if (newIdList != null && newIdList.getSupport() >= minSupportAbsolute) {
                        ...
                        int extItemSize = extension.getItemsSize();
                        extension.setSignOfMatchList(this.i2categHashMap,stepnumber);
                        if (extItemSize > stepnumber || extension.getElements().size() > stepnumber) continue;
...
//                        	System.out.println(kValue+":"+extension.toString());
                        ...
                        if (keepPatterns && extItemSize >= stepnumber) {
            ...
            if (dfs) {
                ...
                if (anyPatternCreated) {
                    execute(child_X, dfs, keepPatterns, verbose, coocMapAfter, coocMapEquals,stepnumber);
                }
            }
        }
        ...
        if (!dfs) {
            ...
            if (anyPatternCreated) {
                for (int i = eqMembers.size() - 1; i >= 0; i--) {
                    ...
                    execute(eqMembers.get(i), dfs, keepPatterns, verbose, coocMapAfter, coocMapEquals,stepnumber);
	...
	public void execute(EquivalenceClass rootClass, boolean dfs, boolean keepPatterns, boolean verbose,
