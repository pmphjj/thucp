...
public abstract class AbstractPetrinetReplayer<T extends Tail, D extends AbstractPDelegate<T>> implements
	...
	public PNRepResult replayLog(final PluginContext context, PetrinetGraph net, final XLog log,
		...
		try {
			...
			int minCostMoveModel = getMinBoundMoveModel(parameters, delta, aStar, localDelegate,log);
...
//			System.out.println(minCostMoveModel);
			...
			Map<Integer,Trace> int2TraceForLabel=new HashMap<Integer,Trace>();
...
//				System.out.println("type "+parameters.getEpsilon());
//				System.out.println("epsilon "+parameters.getType());
...
//						if(trace.getLabel().equals("752B4032D9EC0A485B272CCA1462DAE2")){
//						PRecord test=result.record;
//						PRecord it=test;
...
//						}
			...
			synchronized (col) {
				...
				for(SyncReplayResult myit:col){
					...
					List<StepTypes> stepTypes=myit.getStepTypes();
					SortedSet<Integer> traceIndex=myit.getTraceIndex();
					Trace tempTrace=int2TraceForLabel.get(traceIndex.first());
					w.write(tempTrace.getLabel());
					for(int i=0;i<nodeInstance.size();i++){
						String temp="";
						Object o=nodeInstance.get(i);
						if(o instanceof Transition){
							Transition t=(Transition)o;
							temp+=t.getLabel()+"#";
						}else if(o instanceof XEventClass){
							XEventClass e=(XEventClass)o;
							temp+=e.getId()+"#";
						}
						temp+=stepTypes.get(i).name();
						w.write(","+temp);
					}
					w.newLine();
					Map<String,Double> info=myit.getInfo();
					w2.write(tempTrace.getLabel());
					for(Map.Entry<String, Double> entry:info.entrySet()){ 
						w2.write(","+entry.getKey()+"->"+entry.getValue());
					}
					w2.newLine();
				}
				w.flush();
	...
	protected int getMinBoundMoveModel(IPNReplayParameter parameters, final int delta,
			final MemoryEfficientAStarAlgorithm<PHead, T> aStar, D delegateD,XLog log) throws AStarException {
		// create a log 
		XFactory factory = XFactoryRegistry.instance().currentDefault();
		XTrace emptyTrace = factory.createTrace();
		XEvent s=log.get(0).get(0);
		XEvent e=log.get(0).get(log.get(0).size()-1);
		emptyTrace.add(s);
		emptyTrace.add(e);
...
//			assert (recordD.getCostSoFar() - recordD.getBacktraceSize() - 1) % delta == 0;//ע�͵�by weizhijie
